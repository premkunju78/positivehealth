...
class MarketingheadController extends Controller
{
    public function index(Request $request)
    {
        ...
        $u1 = clone $users;
        $mhs = $u1->limit($request->per_page)->offset(($request->page - 1) * $request->per_page)->get();
        foreach ($mhs as $key => $mh) {
            $alliancepartners = \App\Models\MarketingheadAlliancepartner::where('mh_id', $mh->id)->pluck('alliancepartner_id');
            $alliancepartners = User::find($alliancepartners)->pluck('name');
            $mhs[$key]->alliancepartners = $alliancepartners;
        }
    ...
    public function list()
    {
        ...
        $users = User::join('roles', 'roles.id', '=', 'users.role_id')
            ->leftJoin('user_details as d1', 'users.id', '=', 'd1.user_id')
            ->where('users.role_id', '15')->select('users.id', 'users.name as label')->get();
    ...
    public function store(Request $request, User $user)
    {
        ...
        try {
            ...
            $role_user->role_id = 15;
            $role_user->save();
            $resetUrl = URL::signedRoute('generate-password', ['user' => $user->id]);
            $user->notify(new NewAccountCreated($resetUrl));
            DB::commit();
            return response()->json(['success' => true, 'message' => "Marketing Head {$user->name} created successfully"]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, int $id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                DB::commit();
                return response()->json(['success' => true, 'message' => "Marketing Head {$user->name} updated successfully"]);
            } catch (\Exception $e) {
                DB::rollback();
                return response()->json(['success' => false, 'message' => $e->getMessage()]);
                // return response()->json(['success' => false, 'message' => "Something went wrong please try after some time"]);
        ...
        try {
            if ($request->has('alliance_partner')) {
                ...
                if (empty($check)) {
                    ...
                    return response()->json(['success' => false, 'message' => "Marketing Head already assigned to this alliance partner"]);
                }
            } else {
                return response()->json(['success' => false, 'message' => "Something went wrong please try after some time"]);
            }
    ...
    public function destroy(int $id)
    {
        ...
        if ($user) {
            ...
            DB::table('marketinghead_alliancepartners')->where('mh_id', $user->id)->delete();
