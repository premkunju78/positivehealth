...
class MarketingexecutiveController extends Controller
{
    ...
    public function list()
    {
        ...
        $users = User::join('roles', 'roles.id', '=', 'users.role_id')
            ->leftJoin('user_details as d1', 'users.id', '=', 'd1.user_id')
            ->where('users.role_id', '16')->select('users.id', 'users.name as label')->get();
    ...
    public function store(Request $request, User $user)
    {
        ...
        try {
            ...
            $role_user->role_id = 16;
            $role_user->save();
            $resetUrl = URL::signedRoute('generate-password', ['user' => $user->id]);
            $user->notify(new NewAccountCreated($resetUrl));
            DB::commit();
            return response()->json(['success' => true, 'message' => "Marketing Executive {$user->name} created successfully"]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, int $id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                DB::commit();
                return response()->json(['success' => true, 'message' => "Marketing Executive {$user->name} updated successfully"]);
            } catch (\Exception $e) {
                DB::rollback();
                return response()->json(['success' => false, 'message' => $e->getMessage()]);
                // return response()->json(['success' => false, 'message' => "Something went wrong please try after some time"]);
