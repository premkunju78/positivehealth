...
class DataentryuserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $users = User::join('roles', 'roles.id', '=', 'users.role_id')
            ...
            ->where('users.role_id', '19');

        $u1 = clone $users;
        $dataentryusers = $u1->limit($request->per_page)->offset(($request->page - 1) * $request->per_page)->get();
        return response()->json([
            'dataentryuser' => $dataentryusers,
            'page' => $request->page,
            'total' => $users->count(),
    ...
    public function list()
    {
        ...
        $users = User::join('roles', 'roles.id', '=', 'users.role_id')
            ->leftJoin('user_details as d1', 'users.id', '=', 'd1.user_id')
            ->where('users.role_id', '19')->select('users.id', 'users.name as label')->get();
    ...
    public function store(Request $request, User $user)
    {
        ...
        try {
            ...
            DB::commit();
            return response()->json(['success' => true, 'message' => "Data Entry User {$user->name} created successfully"]);
        } catch (\Exception $e) {
            DB::rollback();
    ...
    public function update(Request $request, int $id)
    {
        ...
        if ($user) {
            ...
            try {
                ...
                DB::commit();
                return response()->json(['success' => true, 'message' => "Data Entry User {$user->name} updated successfully"]);
            } catch (\Exception $e) {
                DB::rollback();
                return response()->json(['success' => false, 'message' => $e->getMessage()]);
                // return response()->json(['success' => false, 'message' => "Something went wrong please try after some time"]);
        ...
        try {
            if ($request->has('alliance_partner')) {
                ...
                if (empty($check)) {
                    ...
                    return response()->json(['success' => false, 'message' => "Data Entry User already assigned to this alliance partner"]);
                }
            } else {
                return response()->json(['success' => false, 'message' => "Something went wrong please try after some time"]);
            }
