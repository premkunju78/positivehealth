    ...
    <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
      ...
      <b-form @submit.prevent="handleSubmit(onSubmit)">
          ...
          <b-col cols="20" md="10">
            ...
            <b-row class="mt-2 mb-2">
              ...
              <b-col cols="12" md="2" class="mt-2 mb-2">
                <b-form-group label="CIS" label-for="cisf">
                  <b-form-checkbox
                    id="cisf"
                    v-model="userData.cisf"
                    name="cisf"
                    value="1"
                    unchecked-value="0"
                  >
                  </b-form-checkbox>
                  <b-form-invalid-feedback v-if="errors.cisf">
                    {{ errors.cisf[0] }}
                  </b-form-invalid-feedback>
                </b-form-group>
              </b-col>
        ...
        <b-row style="margin-top: 5%; margin-left: 40%">
          <!-- Action Buttons -->
          <router-link :to="{ name: 'healthcoaches' }">
            <b-button
              variant="outline-secondary"
              type="reset"
  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        userData.append("cisf", this.userData.cisf);
        if (this.id) {
          data = await axios.post(`/healthcoaches/${this.id}`, userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        } else {
          data = await axios.post("/healthcoaches", userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        ...
        this.$router.replace("/healthcoaches/list");
      } catch (error) {
        if (error.response) {
    ...
    async fetchUserData() {
      try {
        const { data } = await axios.get(`/healthcoaches/${this.id}`);
        this.userData = data.healthcoach;
      } catch (err) {
        console.log(err);
      }
    },
