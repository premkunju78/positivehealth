    ...
    <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
      ...
      <b-form @submit.prevent="handleSubmit(onSubmit)">
        ...
        <b-row style="margin-top: 5%; margin-left: 40%">
          <!-- Action Buttons -->
          <router-link :to="{ name: 'dataentryuser' }">
            <b-button
              variant="outline-secondary"
              type="reset"
  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        if (this.id) {
          data = await axios.post(`/dataentryuser/${this.id}`, userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        } else {
          data = await axios.post("/dataentryuser", userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        ...
        this.$router.replace("/dataentryuser/list");
      } catch (error) {
        if (error.response) {
    ...
    async fetchUserData() {
      try {
        const { data } = await axios.get(`/dataentryuser/${this.id}`);
        this.userData = data.dataentryuser;
      } catch (err) {
        console.log(err);
      }
    },
