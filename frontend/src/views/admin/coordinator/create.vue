  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        if (this.id) {
          data = await axios.post(`/coordinators/${this.id}`, userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        } else {
          data = await axios.post("/coordinators", userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        ...
        this.$router.replace("/coordinators/list");
      } catch (error) {
        if (error.response) {
    ...
    async fetchUserData() {
      try {
        const { data } = await axios.get(`/coordinators/${this.id}`);
        this.userData = data.coordinator;
      } catch (err) {
        console.log(err);
      }
    },
