    ...
    <validation-observer #default="{ handleSubmit }" ref="refFormObserver">
      ...
      <b-form @submit.prevent="handleSubmit(onSubmit)">
        ...
        <b-row style="margin-top: 5%; margin-left: 40%">
          <!-- Action Buttons -->
          <router-link :to="{ name: 'samplecollector' }">
            <b-button
              variant="outline-secondary"
              type="reset"
  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        if (this.id) {
          data = await axios.post(`/samplecollector/${this.id}`, userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        } else {
          data = await axios.post("/samplecollector", userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        ...
        this.$router.replace("/samplecollector/list");
      } catch (error) {
        if (error.response) {
    ...
    async fetchUserData() {
      try {
        const { data } = await axios.get(`/samplecollector/${this.id}`);
        this.userData = data.samplecollector;
      } catch (err) {
        console.log(err);
      }
    },
