  ...
  methods: {
    ...
    async onSubmit() {
      try {
        ...
        if (this.id) {
          data = await axios.post(`/accountsincharge/${this.id}`, userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        } else {
          data = await axios.post("/accountsincharge", userData, {
            headers: {
              "content-type": "multipart/form-data",
            },
          });
        ...
        this.$router.replace("/accountsincharge/list");
      } catch (error) {
        if (error.response) {
    ...
    async fetchUserData() {
      try {
        const { data } = await axios.get(`/accountsincharge/${this.id}`);
        this.userData = data.me;
      } catch (err) {
        console.log(err);
      }
    },
