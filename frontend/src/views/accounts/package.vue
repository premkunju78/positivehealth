        ...
        <b-card v-permission="['view-accounts']">
            ...
            >
                ...
                <template #cell(payment_status)="data">
                    <span class="text-info" v-if="data.item.transaction_date">PAID</span>
                     <span v-else class="text-danger" >UNPAID</span>
                </template>

                 <template #cell(enrolled)="data">
                    <span class="text-info" v-if="data.item.enrolled == 1">YES</span>
...
export default {
    ...
    methods: {
        async getList() {
            try {
                this.loading = true;
                const { data } = await axios.get("packages/details", {
