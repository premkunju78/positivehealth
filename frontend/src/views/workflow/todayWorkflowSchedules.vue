<template>
  <b-card no-body class="mb-0">
      ...
      >
        ...
        <template #cell(status)="data">
          <b-form-group
            ...
            label-cols="10"
            class="status_Text"
            v-if="data.item.user_id == user_id"
          >
        ...
        <template #cell(actions)="data">
          ...
          >
            ...
            <b-dropdown-item
              @click="sendMessageModal(data.item)"
              v-permission="['view-workflows']"
            >
              <feather-icon icon="EyeIcon" />
            ...
            <b-dropdown-item
              @click="viewAssignModal(data.item)"
              v-permission="['view-workflows']"
              v-if="($store.state.auth.user.detail && $store.state.auth.user.detail.incharge == '1' && data.item.status != 'Completed')"
            >
              <feather-icon icon="EyeIcon" />
              <span class="align-middle ml-50">Assign</span>
            </b-dropdown-item>
            <b-dropdown-item
              @click="viewRescheduleModal(data.item)"
              v-permission="['view-workflows']"
            ...
            <b-dropdown-item
              @click="viewmessageModal(data.item)"
              v-permission="['view-workflows']"
            >
              <feather-icon icon="EyeIcon" />
    ...
    <b-modal id="schedule-modal" hide-footer>
      ...
      <b-button class="mt-3" block @click="$bvModal.hide('schedule-modal')"
        >Close Me</b-button
      >
    </b-modal>
                ...
                >
                  <b-form-group label="Message" label-for="message">
                    <b-form-input
                      ...
                      :area-invalid="errors.message ? true : false"
                    />
          ...
          <b-row>
            <b-col cols="12" md="12">
              ...
              >
                ...
                >
                  <v-select
                    v-model="workflowData.consultant_id"
    ...
    <b-modal
      ...
      ok-only
      ok-title="Reschedule"
      @hidden="resetRescheduleData"
      @ok="handleRescheduleOk"
      centered
      size="lg"
  ...
  methods: {
    ...
    async getConsultants() {
      try {
        ...
        var params = {};
        if (
          this.$store.state.auth.user.role_id > 3 && this.$store.state.auth.user.detail &&
